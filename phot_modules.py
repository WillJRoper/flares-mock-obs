...
def lum(sim, kappa, tag, BC_fac, inp='FLARES', IMF='Chabrier_300', LF=True,
    ...
    Lums = {}
    Lums["region_cent"] = cen
    Lums["region_radius"] = radius
    Lums["gal_cop"] = gal_cops
    Lums["gal_ms"] = gal_ms
    Lums["gal_haloids"] = gal_haloids
    ...
    for (num, cop), r in zip(enumerate(cops), r_200):
        ...
        if S_mass[okinds].size < 20:
            continue

        Lums[ind] = {f: {} for f in filters}
        ...
        Lums[ind]["cent"] = cop
        Lums[ind]["smls"] = Smls
        Lums[ind]["masses"] = S_mass[okinds]
        Lums[ind]["part_subgrpids"] = S_subgrpid[okinds]
        ...
        if orientation == "sim":
            ...
            Lums[ind]["coords"] = starCoords - cop
